$(document).ready(function () {
	...
	socket.on('server-broadcast-change-name-group-to-group', function(data){
		...
		$(document).find(".list-message-history").find(".sideBar-body[data-group-id='"+data.group+"']").find(".sideBar-name .name-meta strong").text(data.groupName);
	 ...
	 $(document).on('click', '#btn-click-change-name-group', function(){
	    ...
	    if(group_id != '' && user_change != '' && name_group != ''){
			var message = {
				...
				body: user_change + ' Đã thay đổi tên cuộc trò chuyện nhóm',
				time: new Date(),
				group: group_id,
